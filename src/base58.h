...
public:
    enum
    {
        PUBKEY_ADDRESS = 65,
        SCRIPT_ADDRESS = 85,
        PUBKEY_ADDRESS_TCL = 127,
        SCRIPT_ADDRESS_TCL = 128,
    ...
    bool Set(const CKeyID &id) {
        ...
        {
            SetData(PUBKEY_ADDRESS_TCL, &id, 20);
        }
        else
        {
    ...
    bool Set(const CScriptID &id) {
        if(fTestNet)
        {
            SetData(SCRIPT_ADDRESS_TEST, &id, 20);
        }
        else if(fTCLNet)
        {
            SetData(SCRIPT_ADDRESS_TCL, &id, 20);
        }
        else
        {
    ...
    bool IsValid() const
    {
        ...
        switch(nVersion)
        {
            ...
            case PUBKEY_ADDRESS_TCL:
                nExpectedSize = 20; // Hash of public key
                fExpectTestNet = false;
                break;
            case SCRIPT_ADDRESS_TCL:
                nExpectedSize = 20; // Hash of CScript
                fExpectTestNet = false;
                break;
...
public:
    ...
    { 
        ...
        if(fTCLNet)
        {
            SetData(128 + (CBitcoinAddress::PUBKEY_ADDRESS_TCL), &vchSecret[0], vchSecret.size());
        }
        else
        {
    ...
    bool IsValid() const
    {
        ...
        switch(nVersion)
        {
            ...
            case (128 + CBitcoinAddress::PUBKEY_ADDRESS_TCL):
                break;
