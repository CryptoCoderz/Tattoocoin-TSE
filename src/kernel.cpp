...
// Hard checkpoints of stake modifiers to ensure they are deterministic (TCLNet)
static std::map<int, unsigned int> mapStakeModifierCheckpointsTCLNet =
    boost::assign::map_list_of
        ( 0, 0x0e00670b )
    ;
...
unsigned int GetStakeModifierChecksum(const CBlockIndex* pindex)
{
    ...
    assert (pindex->pprev || pindex->GetBlockHash() == SelectedGenesisNet);
...
bool CheckStakeModifierCheckpoints(int nHeight, unsigned int nStakeModifierChecksum)
{
    if(GetBoolArg("-testnet"))
    {
        MapModifierCheckpoints& checkpoints = mapStakeModifierCheckpointsTestNet;
        if (checkpoints.count(nHeight))
            return nStakeModifierChecksum == checkpoints[nHeight];
    }
    else if(GetBoolArg("-tclnet"))
    {
        MapModifierCheckpoints& checkpoints = mapStakeModifierCheckpointsTCLNet;
        if (checkpoints.count(nHeight))
            return nStakeModifierChecksum == checkpoints[nHeight];
    }
    else
