...
static inline unsigned short GetDefaultRPCPort()
{
    ...
    if(!GetBoolArg("-tclnet"))
        port_number = GetBoolArg("-testnet", false) ? 19960 : 9960;
    if(GetBoolArg("-tclnet"))
        port_number = 9970;
...
int64_t AmountFromValue(const Value& value)
{
    ...
    if(fTCLNet)
    {
    if (dAmount <= 0.0 || dAmount > MAX_MONEY_LE)
        throw JSONRPCError(RPC_TYPE_ERROR, "Invalid amount");
    }
...
Value stop(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "Stop tattoocoin server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "tattoocoin server stopping";
}
...
string HTTPPost(const string& strMsg, const map<string,string>& mapRequestHeaders)
{
    ostringstream s;
    s << "POST / HTTP/1.1\r\n"
      << "User-Agent: tattoocoin-json-rpc/" << FormatFullVersion() << "\r\n"
      << "Host: 127.0.0.1\r\n"
...
void ThreadRPCServer(void* parg)
{
    ...
    RenameThread("tattoocoin-rpclist");
...
void ThreadRPCServer2(void* parg)
{
        ...
        (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"]))
    {
        ...
        string strWhatAmI = "To use tattoocoind";
...
void ThreadRPCServer3(void* parg)
{
    ...
    RenameThread("tattoocoin-rpchand");
